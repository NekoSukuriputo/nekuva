# GEMINI.md — Nekuva (UI App)

> Context file for **Google Antigravity**. Read fully at the start of every session.
> When a rule here conflicts with a prompt, **ask before deviating**. Never silently
> ignore a guardrail in this file.
>
> (Kept in sync with `CLAUDE.md` (Claude Code). The body below is identical across both
> files — only this header differs. Update both when an architectural decision changes.)
