...
class MangaListDecorations(
    ...
    private val savedIds: Set<Long>,
    private val progressEnabled: Boolean,
    private val favouriteBadgeEnabled: Boolean,
    private val savedBadgeEnabled: Boolean,
) {
    fun progressOf(manga: Manga): Float? =
        if (progressEnabled) progressMap[manga.id]?.takeIf { it >= 0f } else null

    fun badgesOf(manga: Manga): MangaBadges = MangaBadges(
        favourite = favouriteBadgeEnabled && manga.id in favouriteIds,
        saved = savedBadgeEnabled && manga.id in savedIds,
    )
}
...
fun rememberMangaListDecorations(includeFavouriteBadge: Boolean = true): MangaListDecorations {
    ...
    val local = koinInject<LocalMangaRepository>()
    val badges = remember { settings.getMangaListBadges() }
    val progressEnabled = remember { settings.progressIndicatorMode != ProgressIndicatorMode.NONE }
    val favBadgeEnabled = remember { includeFavouriteBadge && (badges and 1) != 0 }
    val savedBadgeEnabled = remember { (badges and 2) != 0 }
    ...
    val savedIds by (if (savedBadgeEnabled) local.observeSavedIds() else flowOf(emptySet<Long>()))
        .collectAsState(emptySet())

    return remember(progressMap, favIds, savedIds) {
        MangaListDecorations(progressMap, favIds, savedIds, progressEnabled, favBadgeEnabled, savedBadgeEnabled)
    }
}
