...
import org.nekosukuriputo.nekuva.scrobbling.kitsu.data.KitsuInterceptor.Companion.VND_JSON
...
/**
 * Kitsu OAuth2 (password grant) + JSON:API (port of Doki, ScrobblerConfig client id).
 *
 * Login uses the resource-owner-password flow, so [oauthUrl] is a sentinel (not a real authorize page):
 * the host shows a username/password form and passes `"username;password"` as the `code` to [authorize].
 */
class KitsuRepository(
    ...
    override val oauthUrl: String = "nekuva+kitsu://auth"
    ...
    override suspend fun authorize(code: String?) {
        ...
        val request = Request.Builder().post(body.build()).url("$BASE_WEB_URL/api/oauth/token")
    ...
    override suspend fun loadUser(): ScrobblerUser {
        val request = Request.Builder().get().url("$BASE_WEB_URL/api/edge/users?filter[self]=true")
        val response = okHttp.newCall(request.build()).await().json()
    ...
    override suspend fun unregister(mangaId: Long) =
        db.getScrobblingDao().delete(ScrobblerService.KITSU.id, mangaId)

    override suspend fun findManga(query: String, offset: Int): List<ScrobblerManga> {
        val encoded = URLEncoder.encode(query, "UTF-8")
        val request = Request.Builder().get()
            .url("$BASE_WEB_URL/api/edge/manga?page[limit]=20&page[offset]=$offset&filter[text]=$encoded")
        val response = okHttp.newCall(request.build()).await().json().ensureSuccess()
        return response.getJSONArray("data").mapJSON { jo ->
            ...
            ScrobblerManga(
                ...
                cover = attrs.getJSONObject("posterImage").getStringOrNull("small"),
                url = "$BASE_WEB_URL/manga/${attrs.getString("slug")}",
                isBestMatch = titles.any { it.equals(query, ignoreCase = true) },
            )
        }
    }

    override suspend fun getMangaInfo(id: Long): ScrobblerMangaInfo {
        val request = Request.Builder().get().url("$BASE_WEB_URL/api/edge/manga/$id")
        val data = okHttp.newCall(request.build()).await().json().ensureSuccess().getJSONObject("data")
        ...
        return ScrobblerMangaInfo(
            ...
            url = "$BASE_WEB_URL/manga/${attrs.getString("slug")}",
            descriptionHtml = attrs.getStringOrNull("description").orEmpty().replace("\\n", "<br>"),
        )
    }
    ...
    override suspend fun createRate(mangaId: Long, scrobblerMangaId: Long) {
        ...
        val response = okHttp.newCall(request.build()).await().json().ensureSuccess().getJSONObject("data")
    ...
    override suspend fun updateRate(rateId: Int, mangaId: Long, chapter: Int) {
        val payload = JSONObject().apply {
            putJO("data") {
                put("type", "libraryEntries")
                put("id", rateId)
                putJO("attributes") { put("progress", chapter) }
            }
        }
        val request = Request.Builder()
    ...
    private suspend fun findExistingRate(scrobblerMangaId: Long): JSONObject? {
        ...
        val data = okHttp.newCall(request.build()).await().jsonOrNull()?.optJSONArray("data") ?: return null
    ...
    private fun Response.jsonOrNull(): JSONObject? = use {
