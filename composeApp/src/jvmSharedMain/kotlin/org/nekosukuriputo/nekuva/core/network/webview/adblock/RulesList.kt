...
/**
 * Minimal EasyList parser/matcher (Doki parity — not all features supported). Returns the matching block
 * rule for a URL unless a whitelist (`@@`) rule allows it.
 */
class RulesList {
	...
	private fun String.addImpl(isWhitelist: Boolean, modifiers: String?) {
		...
		when {
			...
			startsWith("##") -> {
				// TODO css rules (cosmetic filtering)
			}

			else -> {
				if (endsWith('*')) {
					list += Rule.Path(this.dropLast(1), contains = true).withModifiers(modifiers)
				} else if (!contains('*')) { // wildcards not supported yet
