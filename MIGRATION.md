...
- [~] `scrobbling` (T2 — **fondasi compile-verified, BELUM run-verify (butuh client ID OAuth dari user)**: `ScrobblerConfig` (placeholder client ID/secret per layanan + `REDIRECT_URI=nekuva://oauth`), model umum (ScrobblerService/Status/User/Manga/MangaInfo/ScrobblingInfo), `ScrobblerStorage` (token di ObservableSettings per-layanan), `ScrobblerRepository` (interface), `Scrobbler` base (port, adaptasi KMP: tanpa parseAsHtml/LongSparseArray). **SISA:** repo+scrobbler konkret per layanan (Shikimori/AniList/MAL/Kitsu) + Discord RPC, alur OAuth (buka oauthUrl di browser in-app → tangkap redirect `code` → authorize), layar Settings→Services (login/logout per layanan), selector "ikat manga ke tracker" di Detail, auto-scrobble saat baca)
